export const buttonOpenPopupProfile = document.querySelector('.profile__edit-button');
export const formProfileInfo = document.querySelector('.popup__form_theme_profile-info');
export const profileNameInput = document.querySelector('.popup__item_el_name');
export const profileJobInput = document.querySelector('.popup__item_el_job');
export const buttonAddCard = document.querySelector('.profile__add-button');
export const formAddCard = document.querySelector('.popup__form_theme_add-card');
export const profileNameField = document.querySelector('.profile__name');
export const profileJobField = document.querySelector('.profile__description');
export const profilePhotoField = document.querySelector('.profile__photo');
export const buttonConfirmDeleting = document.querySelector('.popup__button_theme_confirm');
export const buttonPopupAvatar = document.querySelector('.profile__photo-container');
export const profileAvatar = document.querySelector('.profile__photo');
export const submitAddCard = document.querySelector('.popup__button_theme_add-card');
export const submitProfileInfo = document.querySelector('.popup__button_theme_profile-info');
export const submitPopupAvatar = document.querySelector('.popup__button_theme_edit-photo');
export const formEditAvatar = document.querySelector('.popup__form_theme_edit-photo');